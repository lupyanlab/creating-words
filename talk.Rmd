...
and so convention must also play a role in naming.
-->

# Language is both iconic and arbitrary

- Arbitrariness is built-in to language (Hockett, 1960).
- But even arbitrary languages have iconicity (Perry, Perlman, & Lupyan, 2015).

```{r dimension, engine="dot", eval=FALSE}
digraph {
  rankdir=LR;
  node[fontname=Helvetica shape=circle size=4];
  iconic -> arbitrary[dir=both minlen=2];
  { rank=same; iconic, easy_to_learn; }
  iconic -> easy_to_learn[minlen=2];
  { rank=same; arbitrary, easy_to_generalize; }
  arbitrary -> easy_to_generalize[minlen=2];
  easy_to_learn[label="easy to\nlearn" shape=none];
  easy_to_generalize[label="easy to\ngeneralize" shape=none];
}
```

...
abstract: >
  ...
  rise to more wordlike forms? In what ways do these words resemble the original
...
Imitations were monitored by an experimenter to catch any gross errors in
recording (e.g., loud background sounds) before they were heard by the next
generation of imitators. Recordings were also trimmed to the length of the
utterance. The experimenter also removed recordings that violated the rules of
the experiment, e.g., by saying something in English. A total of `r n_removed`
(`r n_removed_pct`%) imitations were removed prior to subsequent analysis. The
final sample contained `r n_final_imitations` imitations along `r n_branches`
contiguous transmission chains (Fig. 1).
...
On each trial, raters heard two sounds from subsequent generations played
in random order. They then indicated the similarity between the sounds on a
7- point Likert scale from _Entirely different and would never be confused_
to _Nearly identical_. Full instructions are provided in the Supplemental
Materials. Inter-rater reliability was calculated as the intra- class
coefficient treating the group as the unit of analysis [@irr:2012; @Shrout:
1979tg]: `r report_icc_results(irr_results)`. Ratings were normalized for each
...
Participants (_N_=`r n_transcribers`) recruited from Amazon Mechanical Turk were
paid to transcribe vocalizations using English orthography, being instructed
to write down what they heard as a single "word" so that the written word would
sound as much like the sound as possible. Participants were instructed that
this was a word creation task and so to avoid transcribing the vocalizations
...
imitations collected in the transmission chain experiment. Participants also
provided "transcriptions" of the original environmental seed sounds. Analyses of
these transcriptions are reported in the Supplementary Materials (Fig. S5).

To measure similarity among transcriptions of the same imitation, we used the
`SequenceMatcher` functions in the `difflib` package of the Python standard
library, which implements a version of Ratcliff and Obershelp's "gestalt pattern
matching" algorithm. Alternative measures of transcription agreement including
exact string matching and the length of the longest substring match were also
collected.
...
using the Satterthwaite approximation via the `lmerTest` package [@lmerTest:
2016]. Random effects (intercepts and slopes) for subjects and for items were
...
(z-scores) from the generation of sounds. We found that acoustic
...
This result supports the conclusion that transmission chains were stabilizing
on divergent acoustic forms as opposed to all chains converging on similar forms
...
longest contiguous matching subsequences between pairs of transcriptions. We
...
3). The same result is reached through alternative measures of orthographic
...
`r lmer_mod_results(substr_length_mod, "message_c")` (Fig. S3). Differences
between transcriptions of human vocalizations and transcriptions directly of
environmental sound cues are presented in the Supplementary Materials (Fig. S5).
...
to match imitations and their transcriptions back to their original sound source
relative to other seed sounds from either the same category or from different
categories (Fig. 4). We used the match accuracies to answer two questions
concerning the effect of iterated imitation on resemblance to the original seed
sounds. First, we asked whether and for how many generations the imitations
and their transcriptions could be matched back to the original sounds. Second,
we asked whether repeated imitation resulted in a uniform degradation of the
signal in each imitation, or if repeated imitation resulted in some kinds of
information degrading more rapidly than others. Specifically, we tested the
hypothesis that if imitations were becoming more wordlike, then they should
also be interpreted more categorically, and thus we anticipated that imitations
would lose information identifying the specific source of an imitation more
rapidly than category information that identifies the category of environmental
...
Participants in the True seed and Category match conditions were presented
sounds from different categories as choices. Participants in the Specific match
condition were presented four sounds from the same category.
...
"transcriptions" directly of the environmental sounds are shown in Fig. S5.
...
One consequence of imitations becoming more wordlike is that they may make
for better category labels. For example, an imitation from a later generation,
by virtue of having a more wordlike form, may be easier to learn as a label
...
and transcription procedure, we sampled `r n_lsn_words` words transcribed from
first and last generation imitations, including transcriptions of the original
seed sounds, that were equated in terms of length and association strength with
the original sounds. Our procedure for sampling transcriptions is detailed in
...
block of 24 trials was 81%; Fig. S4). Participants learning transcriptions
...
benefit to the stabilization of repeated imitations on more wordlike forms.
As a consequence of being more wordlike, repeated imitations were responded
...
became more wordlike both in form and function. In form, the vocalizations
...
into the English alphabet. Even as the vocalizations became more wordlike, they
...
This was approved by the University of Wisconsin's Educational
and Social/Behavioral Sciences Institutional Review Board in accordance with the

...
We report the results of the same model fit to two datasets: one containing all question data and a subset with outlier questions (questions with 100% agreement--either correct or incorrect) removed. Questions with 100% agreement were considered outlier questions because in some cases, e.g., when all participants select the same incorrect response, mean matching accuracy dropped below chance levels, causing the model to predict below chance levels, violating an assumption of our analysis approach. Removing these outlier questions caused the model predictions to be more interpretable without changing the substantive conclusions drawn from the model parameters and interaction terms. We report the model results from the full model first, followed by the same terms from the model without outlier questions. The model predictions from the full model are available in the Supporting Information.

Fig. 4 shows the changes in matching accuracy in each question type over generations. Performance decreased over generations more rapidly for questions requiring a within-category distinction than for between-category questions, full model: `r glmer_mod_results(imitation_matches_mod, "generation_1:same_v_within")`, no outliers: `r glmer_mod_results(imitation_matches_noperfect_mod, "generation_1:same_v_within")` suggesting that between-category information was more resistant to loss through transmission. One explanation for this result is that the within-category match questions are simply more difficult because the sounds are more acoustically similar to one another than the between-category questions and therefore performance might be expected to drop off more rapidly with repeated imitations. However, performance also decreased for the easiest type of question where the correct answer was the actual seed generating the imitation (True seed questions; see Fig. 3); the advantage of having the true seed among between-category distractors decreased over generations, full model: `r glmer_mod_results(imitation_matches_mod, "generation_1:same_v_between")`, no outliers: `r glmer_mod_results(imitation_matches_noperfect_mod, "generation_1:same_v_between")`.

...
abstract: |
  ...
  they could be matched above chance to the environmental sound that motivated
...
We collected a total of `r n_all_imitations` imitations, of which `r n_removed` were removed, leaving `r n_final_imitations` imitations along `r n_branches` contiguous transmission chains for analysis. Imitations from later generations were rated as being more similar to one another than imitations from earlier generations, `r lmer_mod_results(similarity_judgments_lmertest_mod, "edge_generation_n")` (Fig. 2), suggesting that the imitations stabilized through repetition.
...
## Transcriptions retained resemblance to seed category

```{r matching-transcriptions, include=FALSE}
```

The top 4 most frequent transcriptions for each transcribed imitation were tested in a modified version of the "Guess the Seed" game. Instead of listening to imitations, participants now read a transcription of an imitation, which they were told was an invented word. They were instructed that the word was invented to describe one of four presented sounds, and they had to guess which one.
